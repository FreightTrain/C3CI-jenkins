  ...
  module Tasks
    describe DownloadCli do
      describe '#execute' do
        ...
        before do
          allow(Net::HTTP).to receive(:get_response).with('localhost', '/jnlpJars/jenkins-cli.jar', 8088).and_return(response)
          allow(Dir).to receive(:tmpdir).and_return('/tmp')
        end
        it 'fetches the jenkins cli jar' do
          expect(Net::HTTP).to receive(:get_response).with('localhost', '/jnlpJars/jenkins-cli.jar', 8088).and_return(response)
          task.execute
        end
        it 'raises if the download response is a 500' do
          ...
          allow(Net::HTTP).to receive(:get_response).with('localhost', '/jnlpJars/jenkins-cli.jar', 8088).and_return(error_response)
